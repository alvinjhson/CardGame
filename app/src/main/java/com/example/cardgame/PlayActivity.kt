...
class PlayActivity : AppCompatActivity() {
    ...
    override fun onStop() {
        ...
        with (sharedPref.edit()) {
            ...
            putInt("playerBalance",playersList[0].balance)
    ...
    fun checkBlackJack() {
        if (checkWin == false) {
            if (playerValue == 21) {
                ...
                checkWin = true
                blackJack = true
                winBet(playerPot, playersList[0])
                resetGame()
    ...
    fun checkDraw(){
        if (playerValue == dealerValue) {
            ...
            draws++
            draw = true
            playersList[0].balance += playerPot
            resetGame()
        }
    }
    fun firstCards(){
   ...
   fun dealerAdjustAceValue() {
    ...
    fun doubleBet() {
        ...
        val standButton = findViewById<Button>(R.id.standButton)
        val doubleButton = findViewById<Button>(R.id.doubleButton)
        if (playersList[0].balance > playerPot) {
            hit()
            playersList[0].balance -= playerPot
    ...
    fun winBet(bet : Int,player: Player) {
        var win = (bet * 2)
        var blackJackWin = (bet * 2.5).toInt()
        if (blackJack == true){
            player.balance += blackJackWin
            totalChipsWon += blackJackWin
        }else{
            player.balance += win
            totalChipsWon += win
        }
    ...
    fun playerBalanceTextView() {
        balanceTextView.text = "${playersList[0].balance} $"
    }
    fun resetGame() {
        val bet20Button = findViewById<ImageButton>(R.id.chip20Button)
        val bet50Button = findViewById<ImageButton>(R.id.chip50Button)
